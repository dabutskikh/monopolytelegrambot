...
public class TransferToPlayerHandler implements TextCommandHandler {
    ...
    public List<Response> execute(CommandContext context) {
        ...
        playerGameDto.setState(PlayerGameState.SELECT_PLAYER);
        playerGameService.update(playerGameDto);
        List<PlayerDTO> allPlayers = playerService.findActiveGamePlayers(playerGameDto.getGameId());
        return Collections.singletonList(new Response(context.getUserId(),
                """
                        Перевод игроку
                        Выберите игрока
                        """,
                Keyboards.getWithRows(allPlayers.stream()
