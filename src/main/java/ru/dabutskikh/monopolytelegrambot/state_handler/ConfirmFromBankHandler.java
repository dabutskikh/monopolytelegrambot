...
public class ConfirmFromBankHandler implements PlayerGameStateHandler {
    ...
    public PlayerGameState getKey() {
        return PlayerGameState.CONFIRM_FROM_BANK;
    }
    ...
    private List<Response> commitTx(GameMoveContext context, TxDTO txDto) {
        ...
        String toYourselfMessage = String.format("""
                        Вы получили из банка %s
                        Ваш баланс: %s
                        """,
                amount,
                playerGameDto.getMoney()
        );
        String toOtherPlayersMessage = String.format("Игрок %s получил из банка %s", username, amount);
